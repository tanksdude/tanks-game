...
int KeypressManager::keyCodeFromString(std::string key) {
...
void KeypressManager::unsetKeyState(std::string key) {
	keyStates[keyCodeFromString(key)] = false;
}

void KeypressManager::keyCallbackFunc(GLFWwindow*, int key, int scancode, int action, int mods) {
	switch (action) {
		case GLFW_PRESS:
			keyStates[key] = true;
			break;
