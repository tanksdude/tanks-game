...
class LightningCornersLevel : public Level {
	//cut from JS Tanks 'cuz it didn't have anything going for it
	//I think it was originally made to play around with powerup mixing
public:
	std::string getName() const override { return "lightning_corners"; }
	ColorValueHolder getDefaultColor() const override { return ColorValueHolder(0x77/255.0, 0x55/255.0f, 0.75f); }
